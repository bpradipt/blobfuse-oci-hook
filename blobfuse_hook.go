...
import (
	...
	"github.com/bpradipt/blobfuse-oci-hook/internal"
...
func main() {
	...
	rootCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if version {
				fmt.Printf("blobfuse oci hook version %s\n", Version)
				os.Exit(0)
			}
			...
			if logFile != "" {
				...
				dname, err := os.MkdirTemp("", "blobfuse_hook_log")
				if err != nil {
					log.Fatal(err)
				}
			...
			log.Infof("Started blobfuse oci hook version %s\n", Version)
